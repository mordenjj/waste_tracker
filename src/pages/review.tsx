...
function getDateRange(preset: DatePreset): { recordedAt?: string[] } {
  ...
  // We use the actual column name 'recordedAt' as the key
  const formatRange = (days: number) => [
    `gte.${startOfDay(subDays(now, days)).toISOString()}`,
    `lte.${now.toISOString()}`
  ];

  if (preset === "7days") return { recordedAt: formatRange(7) };
  if (preset === "30days") return { recordedAt: formatRange(30) };
  if (preset === "90days") return { recordedAt: formatRange(90) };
